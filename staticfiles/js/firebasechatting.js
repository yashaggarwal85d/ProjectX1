...
const ProjectLastMessage = document.getElementsByClassName(
  'project_lastmessage'
);
class chat {
  ...
  buildDockey() {
    ...
    else var dockey = String(this.sender_id) + ':' + String(this.user_id);
  ...
  buildchat = async () => {
    ...
    firebaseRef.onSnapshot((value) => {
      if (typeof value.data() != 'undefined') {
        var filterArray = value
          .data()
          .users.filter((_user) => _user == this.user_id)[0]
          .split('')[0];
        ...
        if (this.user_id == filterArray && this.isactive) {
          for (message of this.messages) {
            ...
            if (
              this.search_message !== '' &&
              message.message.indexOf(this.search_message) === -1
            ) {
            } else {
...
class Issuechat {
  ...
  buildIssue() {
    ...
    var url = `${API}/issues/issue_detail_api/${this.issue_id}/`;
    $.get(url, (data) => {
      ...
      if (typeof value.data() != 'undefined') {
        ...
        if (this.issue_id == value.data().issue && this.isactive) {
          for (message of this.messages) {
            ...
            wrapper.innerHTML = item;
            const issue_user_img = document.getElementsByClassName(
              `issue_user_img_${message.sender}`
            );
            const issue_user_name = document.getElementsByClassName(
              `issue_user_name_${message.sender}`
            );
            var url = `${API}/accounts/profile_api/${message.sender}/`;

            $.get(url, (data) => {
              var i;
              for (i = 0; i < issue_user_img.length; i++) {
                issue_user_img[i].src = data.profile_pic;
                issue_user_name[i].innerHTML =
...
class Projectchat {
  ...
  buildProject() {
    ...
    var url = `${API}/projects/project_detail_api/${this.project_id}/`;
    $.get(url, (data) => {
      console.log(data);
      ProfilePic.src = `https://picsum.photos/id/${this.project_id}/200/300`;
      ProfileName.innerHTML = data.name;
      ProfileSkills.innerHTML = `Description : ${data.description.replace(
        /<[^>]*>/g,
        ''
      )}`;
    });
      ...
      if (typeof value.data() != 'undefined') {
        ...
        if (this.project_id == value.data().project && this.isactive) {
          for (message of this.messages) {
            ...
            wrapper.innerHTML = item;
            const project_user_img = document.getElementsByClassName(
              `project_user_img_${message.sender}`
            );
            const project_user_name = document.getElementsByClassName(
              `project_user_name_${message.sender}`
            );
            var url = `${API}/accounts/profile_api/${message.sender}/`;
  ...
  await firebase
    ...
    .onSnapshot((value) => {
      ...
      chats.map((_chat, _index) => {
        const lastmessage = _chat.messages[
          _chat.messages.length - 1
        ].message.substring(0, 20);
        const time = moment(
          _chat.messages[_chat.messages.length - 1].timestamp
        ).fromNow();
        const secondUser = _chat.users
          .filter((_user) => _user !== user)[0]
          .split('')[0];
        ...
        var url = `${API}/accounts/profile_api/${secondUser}/`;
        $.get(url, (data) => {
          const profileimg = document.getElementById(
            `profile_pic_${secondUser}`
          );
          const profilename = document.getElementById(
            `profile_name_${secondUser}`
          );
          ...
          if (
            ...
            _chat.messages[_chat.messages.length - 1].sender != user &&
            ActiveChatReturn() != secondUser
          ) {
...
ListAllIssues = async () => {
  ...
  for (i = 0; i < IssueTime.length; i++) {
    ...
    firebaseRef.onSnapshot((value) => {
      ...
      ThisIssueLastMessage.innerHTML = lastissuemessage.message.substring(
        0,
        20
      );
  ...
  for (i = 0; i < ProjectTime.length; i++) {
    ...
    firebaseRef.onSnapshot((value) => {
      ...
      const LastMessageTimestamp = moment(
        lastprojectmessage.timestamp
      ).fromNow();
      ThisProjectLastMessage.innerHTML = lastprojectmessage.message.substring(
        0,
        20
      );
...
function TriggerOnChange(e) {
  if (chat_instance.isactive) OnchangeChats(e);
  else if (issue_instance.isactive) OnchangeIssues(e);
  else if (project_instance.isactive) OnchangeProjects(e);
}
